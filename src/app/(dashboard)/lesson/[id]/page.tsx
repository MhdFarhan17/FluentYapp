    ...
    const fetchExercises = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          toast.error("No exercises available for this lesson yet!");
        }
      } catch (err) {
  ...
  const handleNext = async () => {
    if (currentIndex < exercises.length - 1) {
      ...
      try {
        ...
        if (user) {
          // Anti-Abuse: Check if lesson was already completed
          const isReview = completedLessons.includes(id);
          setWasReview(isReview);
          
          const maxPossible = exercises.length * 10;
          const penalty = mistakes * 5;
          const calculatedXp = Math.max(10, maxPossible - penalty);
          const xpEarned = isReview ? 10 : calculatedXp; // Flat 10 XP for review
          ...
          if (!isReview) {
            ...
            // Insert completed lesson directly on client to pass RLS
            const { error: insertError } = await supabase
              .from("user_completed_lessons")
              .insert({ user_id: user.id, lesson_id: id });
              
            if (insertError && insertError.code !== '23505') {
              console.error("Failed to mark lesson as completed:", insertError);
            }
          }
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center animate-in fade-in zoom-in duration-700 p-4">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold mb-4 text-center text-foreground tracking-tight">
          {wasReview ? "Review Complete!" : "Lesson Complete!"}
        </h1>
        
        {wasReview ? (
          <p className="text-2xl text-foreground/50 font-extrabold mb-8 text-center flex items-center space-x-2 bg-secondary/50 px-6 py-3 rounded-full border-2 border-border border-dashed">
            <span>+{displayXp} XP (Review Bonus)</span>
          </p>
        ) : (
